...
export function twemojiParseWithLinks(input: string, elementClass?: string): string {
  const urlRegex = /http(s)?:\/\/([a-z0-9.-]+){1,3}(:[0-9]+)?(\/[\w?%.\-_@&]*)?/g; // Updated to allow for proper URL parsing
  ...
  Array.from(processedText ?? '').forEach((char) => {
    if (emojiRegex.test(char)) {
      ...
      result += `<img src="${imgUrl}" style="height: 1em; width: 1em; margin: 0 .05em 0 .1em" alt="${char}" aria-hidden="true"/>`;
    } else result += char;
  });
